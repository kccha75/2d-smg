...
% pde.c
% pde.f
% scheme.dx - step size in x
% scheme.dy - step size in y
% option.numit - number of iterations
%
% Ouputs:
% v - best estimate after relaxation

function v=fourier_RBGSLineRelax(v,pde,scheme,option)

for i=1:option.numit
    
    % Line Relax y direction
    v  = fourier_RBGSLineRelaxY(v,pde,scheme);
...
function v = fourier_RBGSLineRelaxY(v,pde,scheme)
...
for n=1:2
    ...
    % Generate RHS
    g(:,col)=pde.f(:,col)+1/scheme.dy^2*(b_mid(:,col).*v(:,col1)+b_mid(:,col2).*v(:,col2));
    
    % Generate LHS tridiagonal matrix + solve
    for i=1:length(col)
        
        B=zeros(Nx,3);
        B(index2x,2)=(a_mid(index2x,col(i))+a_mid(index3x,col(i)))/scheme.dx^2+(b_mid(:,col(i))+b_mid(:,col1(i)))/scheme.dy^2+pde.c(:,col(i)); % main diag
        B(1:Nx-1,1)=-a_mid(1:Nx-1,col(i))/scheme.dx^2; % lower diag
        B(2:Nx,3)=-a_mid(1:Nx-1,col(i))/scheme.dx^2; % upper diag
    
        A=spdiags(B,-1:1,Nx,Nx);
        
        % Periodic condition
        A(1,Nx)=-a_mid(Nx,col(i))/scheme.dx^2;
        A(Nx,1)=-a_mid(Nx,col(i))/scheme.dx^2;
