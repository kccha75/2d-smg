...
function [V,U,W]=naturalparametercontalphaDJL_test(v,delta,alpha,DJL,domain,option,cont_option)
...
%     DJL.u=W(j+1);
    ...
    if j<=2 % at first step 
        ...
        % Update secant 
        [V(:,:,j+1),W(j+1),y,numsecantit,secantflag]=DJLtabletopsecant(v1,v2,W(j)+dw1*ds,W(j)+1e-5,DJL,pde,domain,option);
        ...
        [v2,~,newtonflag2]=NewtonSolve(v1,DJL,pde,domain,option);
        % Update secant 
        [V(:,:,j+1),W(j+1),y,numsecantit,secantflag]=DJLtabletopsecant(v1,v2,W(j)+dw1*ds,W(j)+dw2*ds,DJL,pde,domain,option);
    end
