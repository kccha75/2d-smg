...
function [DJL,fKdV,pdefkdv,domainfkdv,optionfkdv]=DJLv0_2topography_neg(DJL,domain,option)
...
if DJL.soltype==3 % 2 bump fKdV continuation solitary wave +ve branch

    load gamma_cont_neg.mat
    load lambda_cont_neg.mat
    load B_cont_neg.mat
    Bindex=findroot(gamma_cont_neg,gamma_star);

    % Linear Interpolation
    B=(gamma_star-gamma_cont_neg(Bindex))*(B_cont_neg(:,Bindex+1)-B_cont_neg(:,Bindex)) ...
        /(gamma_cont_neg(Bindex+1)-gamma_cont_neg(Bindex))+B_cont_neg(:,Bindex);

    delta_star=(gamma_star-gamma_cont_neg(Bindex))*(lambda_cont_neg(Bindex+1)-lambda_cont_neg(Bindex)) ...
        /(gamma_cont_neg(Bindex+1)-gamma_cont_neg(Bindex))+lambda_cont_neg(Bindex);
    ...
    % Newton solve???
    % maybe later ... i don't want to redo code ;(
...
b=DJL.topography(X); % probably use topography here ...
