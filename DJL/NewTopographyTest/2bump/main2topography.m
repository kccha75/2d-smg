...
% N^2 function
N2=@(psi) exp(-1.0*psi)/((exp(-1.0)-1)/-1.0);N2=@(psi) 2*(-psi+1);%N2=@(psi) sech(2*(psi-0)).^2/(tanh(1));
% (N^2)'
N2d=@(psi) -1.0*exp(-1.0*psi)/((exp(-1.0)-1)/-1.0);N2d=@(psi) -2+0*psi;%N2d=@(psi) -4*sech((psi-0)).*tanh((psi-0))/(tanh(1));
