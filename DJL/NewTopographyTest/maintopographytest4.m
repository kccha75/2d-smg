...
% N^2 function
N2=@(psi) sech((psi-1)/.9).^2;%N2=@(psi) psi;

% (N^2)'
N2d=@(psi) -2*sech((psi-1)/.9).^2.*tanh((psi-1)/.9);%N2d=@(psi) 1+0*psi;
...
% an
an=pagemtimes(A_xx,DJL.a1)+pagemtimes(A.^2,DJL.a2)+pagemtimes(alpha*DJL.b,DJL.a3);
