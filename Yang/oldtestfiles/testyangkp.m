...
% FOR +ve
% lowest grid is N=8 for smg ... when N*(N-3) is used
% as low as 5 for (N-1)*N grid ...
% N*N grid can only use 8 ...
% FOR -ve
% when N*(N-3) is used only can use 8
% (N-1)*N grid can only use 8 ...
...
for i=1:20
    ...
    option.tol=max(option.tol,mg_tol)/r;
