...
% Uses N*(N-3) grid size
% Very very rough code ... is not clean
% -ve initial condition can use k=4, coarsestgrid 9 finestgrid 10
% -------------------------------------------------------------------------
...
semilogy(M,t_cg,'-x',M,t_mg(:,4),'-o',M,t_mg(:,3),'-o',M,t_mg(:,2),'-o',M,t_mg(:,1),'-o')
xlabel('$N$','interpreter','latex','fontsize',fsz)
ylabel('$t$','interpreter','latex','fontsize',fsz)
legend('CG','SMG coarse grid N=6','SMG coarse grid N=7','SMG coarse grid N=8','SMG coarse grid N=9','Location','NorthWest')