...
% -------------------------------------------------------------------------
% Yang paper example 3.5 second part
% coarse grid cannot go below 7, 7 seems best 
% -------------------------------------------------------------------------
...
V0=6;mu=4.56;
c=@(X,Y) -V0*(sin(X*10).^2+sin(Y*10).^2)+mu;

% RHS
f=@(X,Y) 0*X;

% Initial guess
v0=@(X,Y) 0.56*sech(0.5*sqrt((X*10).^2+(Y*10).^2)).*cos(X*10).*cos(Y*10);
