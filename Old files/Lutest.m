...
function f=Lutest(v,pde,domain)
...
u{1}=reshape(v,domain.N(1),domain.N(2));
if domain.dim==2;u{2}=u{1}';end
...
for i=1:domain.dim
    ...
    if domain.discretisation(i)~=1
        f(index{1,i})=domain.BC{1,i}.*u{1}(index{1,i})+ ...
            domain.BC{2,i}.*sum(fct(u{i}).*k{i}.^2); % x(1)
        f(index{2,i})=domain.BC{3,i}.*u{1}(index{2,i})+ ... 
